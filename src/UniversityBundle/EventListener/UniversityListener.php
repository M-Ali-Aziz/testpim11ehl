<?php
namespace UniversityBundle\EventListener;
...
/**
 * This is the class that contain the EventListener functions for University DataObjact.
 */
class UniversityListener {
     
    public function onUniversitySlugifyName (ElementEventInterface $e)
    {
        if($e instanceof DataObjectEvent) {
            ...
            if (method_exists($object, 'getClassName')) {
                if ($object->getClassName() == 'University') {
                    $title = $object->getUniversity();
                    ...
                    $index = 0;
                    while( $keyExist ) {
                        $list = new DataObject\University\Listing();
